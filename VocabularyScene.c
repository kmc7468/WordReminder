...
LRESULT CALLBACK VocabularySceneProc(HWND handle, UINT message, WPARAM wParam, LPARAM lParam, UINT_PTR dummy0, DWORD_PTR dummy1) {
	...
	case AM_CREATEUI: {
		...
		UIFONT_CON(staticFont, 18, true);
		UIFONT_CON(textFont, 18, false);

		UICOMP_DOH(section, Horizontal, None, 100, &uiEngine->RootComponent);
		UIMARG_CON_AS(section, 10);

		UICOMP_CON_W(wordListStatic, &g_WordListStatic, Horizontal, None, 23, staticFont, section);
		UIMARG_CON(wordListStatic, Bottom, 5);
		UICOMP_DOW_W(wordList, &g_WordList, Vertical, None, 33, textFont, section);
		UIMARG_CON(wordList, Right, 5);

		UICOMP_WIN(wordSection, None, section);
		UIMARG_CON(wordSection, Left, 5);

		UICOMP_CON_W(wordStatic, &g_WordStatic, Horizontal, None, 23, staticFont, wordSection);
		UIMARG_CON(wordStatic, Bottom, 5);
		UICOMP_CON_W(wordEdit, &g_WordEdit, Horizontal, None, 35, textFont, wordSection);
		UIMARG_CON(wordEdit, Bottom, 10);

		UICOMP_WIN(meaningSection1, None, wordSection);
		UIMARG_CON(meaningSection1, Bottom, -60);

		UICOMP_CON_W(meaningListStatic, &g_MeaningListStatic, Horizontal, None, 23, staticFont, meaningSection1);
		UIMARG_CON(meaningListStatic, Bottom, 5);
		UICOMP_DOW_W(meaningList, &g_MeaningList, Vertical, None, 50, textFont, meaningSection1);
		...
		UICOMP_CON_W(meaningStatic, &g_MeaningStatic, Horizontal, None, 23, staticFont, meaningSection2);
		UIMARG_CON(meaningStatic, Bottom, 5);
		UICOMP_CON_W(meaningEdit, &g_MeaningEdit, Horizontal, None, 35, textFont, meaningSection2);
		UIMARG_CON(meaningEdit, Bottom, 10);
		UICOMP_CON_W(pronunciationStatic, &g_PronunciationStatic, Horizontal, None, 23, staticFont, meaningSection2);
		UIMARG_CON(pronunciationStatic, Bottom, 5);
		UICOMP_CON_W(pronunciationEdit, &g_PronunciationEdit, Horizontal, None, 35, textFont, meaningSection2);
		UIMARG_CON(pronunciationEdit, Bottom, 10);
		UICOMP_CON_W(addMeaningButton, &g_AddMeaningButton, Horizontal, None, 55, staticFont, meaningSection2);
		UIMARG_CON(addMeaningButton, Bottom, 5);
		UICOMP_CON_W(removeMeaningButton, &g_RemoveMeaningButton, Horizontal, None, 50, staticFont, meaningSection2);

		UICOMP_CON_W(removeWordButton, &g_RemoveWordButton, Horizontal, None, 60, staticFont, meaningSection1);
		...
		UICOMP_DOW_W(loadVocabularyButton, &g_LoadVocabularyButton, Vertical, None, 50, staticFont, vocabularyButtonSection);
		UIMARG_CON(loadVocabularyButton, Right, 5);
		UICOMP_DOW_W(saveVocabularyButton, &g_SaveVocabularyButton, Vertical, None, 50, staticFont, vocabularyButtonSection);
