...
DWORD WINAPI JoinServerThread(LPVOID param) {
	...
	if (!JoinServer(&Context, option)) {
		MessageBox(Context.Window, _T("������ �����ϴ� �� ������ �߻��߽��ϴ�."), _T("����"), MB_OK | MB_ICONERROR);
	...
	if (!SendInt(&Context, StartCode) ||
		...
		_tcscpy(message, _T("������ ������ Ŭ���̾�Ʈ�� ������ ȣȯ���� �ʽ��ϴ�.\n"));
		_tcscat(message, serverVersion);
		_tcscat(message, _T(" ������ �ܾ� �ϱ� ���α׷��� ��ġ�� ������."));
		MessageBox(Context.Window, message, _T("����"), MB_OK | MB_ICONERROR);
		free(serverVersion);
		free(message);
...
DWORD WINAPI ReceiveThread(LPVOID param) {
	...
	while (ReceiveInt(&Context, &magic)) {
		...
		ConnectionError:
			...
			const bool accept = MessageBox(Context.Window, _T("������ ���� ������ ��û�߽��ϴ�. �����Ͻðڽ��ϱ�?"), _T("����"), MB_YESNO | MB_ICONQUESTION) == IDYES;
			SendInt(&Context, DenyChangeRoleCode - accept);
			if (accept) {
				SendMessage(Context.Window, WM_USER + 8, 0, 0);
			}
		} else if (magic == AcceptChangeRoleCode) {
			MessageBox(Context.Window, _T("������ ���� ������ �����߽��ϴ�."), _T("����"), MB_OK | MB_ICONINFORMATION);
